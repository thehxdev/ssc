...
typedef struct {
    ...
    // temporary buffer
    long tmppos;
    char tmpbuf[SSC_FULL_BUFFER_SIZE];
} ssc_session_t;

// one arena; no malloc/free hell!
static sth_arena_t *arena;
static sth_mempool_t bufpool;
static sth_mempool_t session_pool;
static sth_mempool_t wrreq_pool;
...
static void buf_alloc_cb(uv_handle_t *handle, size_t suggested_size, uv_buf_t *buf) {
    buf->base = sth_mempool_get(&bufpool);
    buf->len  = SSC_FULL_BUFFER_SIZE;
}
    ...
    switch (s->client_stage) {
        ...
        case CLIENT_STAGE_HANDSHAKE: {
...

            {
                ...
                //////
                ptr = 0;
                uint8_t *vheader = sth_arena_alloc(arena, vheader_length);

                // set destination address type, address and port in variable-length header
                memcpy(vheader, s->tmpbuf, s->tmppos);
                ptr += s->tmppos;
                s->tmppos = 0;
                ...
                *((uint16_t*)&vheader[ptr]) = htons(padding_length);
                ...
                if (with_initial_payload) {
                    ...
                    // completed
                    memcpy(s->tmpbuf, rdbuf->base, nread);
                    s->tmppos = nread;
                }
...
ret:
    sth_mempool_put(&bufpool, rdbuf->base);
    return;
    ...
    switch (s->remote_stage) {
        case REMOTE_STAGE_HANDSHAKE: {
            ...
            if (s->tmppos > 0) {
                ...
                uint16_t n_ne16 = htons(s->tmppos);
                ok = ssc_crypto_encrypt(&s->crypto,
                                        wrreq->buf.base, &encrypted_size,
                                        &wrreq->buf.base[sizeof(uint16_t)], TAG_SIZE,
...
static void remote_connect_cb(uv_connect_t *req, int status) {
    ...
    ssc_write_req_t *wrreq = sth_mempool_get(&wrreq_pool);
    wrreq->buf = (uv_buf_t){
        .base = s->socksreply,
        .len = 10
    };
...
int main(int argc, char *argv[]) {
    ...
    sth_arena_config_t aconf = STH_ARENA_DEFAULT_CONFIG;
    aconf.flags   = STH_ARENA_FIXED;
    aconf.reserve = STH_BASE_MB(128ULL);
    aconf.commit  = STH_BASE_MB(32ULL);
    if ( !(arena = sth_arena_new(aconf)))
        goto ret;

    sth_mempool_init(&bufpool, arena, SSC_FULL_BUFFER_SIZE);
    sth_mempool_init(&session_pool, arena, sizeof(ssc_session_t));
    sth_mempool_init(&wrreq_pool, arena, sizeof(ssc_write_req_t));
    ...
    uv_tcp_t *socks_server = sth_arena_alloc(arena, sizeof(*socks_server));
